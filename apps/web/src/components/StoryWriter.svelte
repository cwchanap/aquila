<script lang="ts">
    ...
    // Form states
    let storyForm = $state<{ title: string; description: string; status: 'draft' | 'published' | 'archived' }>({ 
        title: '', 
        description: '', 
        status: 'draft' 
    });
    let chapterForm = $state({ title: '', description: '', storyId: '' });
    let sceneForm = $state({ title: '', content: '', storyId: '', chapterId: '' });
...
<div class="story-writer">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    ...
                    <div class="space-y-2">
                        {#each stories as story (story.id)}
                            ...
                            >
                                ...
                                <div class="text-xs text-gray-400 mt-1 capitalize">{story.status}</div>
                            </button>
                        {/each}
                    </div>
                {/if}
        ...
        <div class="lg:col-span-3">
            {#if selectedStory}
                ...
                />
            {:else}
                <div class="bg-white/5 backdrop-blur-sm rounded-lg p-12 border border-white/10 text-center">
                    <BookOpen size={64} class="mx-auto mb-4 text-gray-500" />
                    <p class="text-gray-400 text-lg">Select a story to view its structure</p>
                </div>
            {/if}
        </div>
    </div>
    ...
    <!-- Modals -->
    <Modal bind:open={showStoryModal} title={editMode === 'story' ? 'Edit Story' : 'Create New Story'}>
        <StoryForm
