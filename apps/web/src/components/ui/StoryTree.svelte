...
<div
  class="story-tree bg-white/5 backdrop-blur-sm rounded-lg p-4 border border-white/10"
>
  <!-- Story Root -->
  <div class="story-root mb-4">
    <div
      class="flex items-center justify-between p-3 bg-linear-to-r from-purple-500/20 to-blue-500/20 rounded-lg border border-purple-300/30 hover:border-purple-300/50 transition-colors"
    >
    ...
    <div class="pl-8">
      ...
      {#each story.chapters.sort((a, b) => a.order - b.order) as chapter (chapter.id)}
        <div class="chapter-item mb-3">
          <div
            class="flex items-center justify-between p-2 bg-white/5 rounded-lg border border-white/10 hover:border-white/20 transition-colors"
          >
            <div class="flex items-center gap-2 flex-1">
              <button
              ...
              <span class="text-xs text-gray-400"
                >({chapter.scenes.length} scenes)</span
              >
            </div>
            <div class="flex items-center gap-2">
              <button
          ...
          {#if expandedChapters.has(chapter.id)}
            <div class="pl-8 mt-2 space-y-2">
              {#each chapter.scenes.sort((a, b) => a.order - b.order) as scene (scene.id)}
                <div
                  class="flex items-center justify-between p-2 bg-white/5 rounded border border-white/5 hover:border-white/10 transition-colors"
                >
                  <div class="flex items-center gap-2">
