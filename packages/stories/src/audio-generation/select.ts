...
export async function selectAudioCandidate(
    ...
    // The design assumes one local operator running sequential CLI commands,
    // so there is no cross-process lock. The temp-file + rename pair still
    // makes the write atomic: a partial write never replaces selection.json.
